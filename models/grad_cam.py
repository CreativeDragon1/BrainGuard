...
try:
    ...
    cv2 = None
    print("⚠️ OpenCV not available - Grad-CAM visualization will be limited")
...
class GradCAM:
    ...
    def generate(self, input_tensor, target_class=None):
        ...
        if cv2 is not None:
            ...
            cam = np.uint8(255 * cam)
            # Simple heat map without colormap
            cam_colored = np.zeros((224, 224, 3), dtype=np.uint8)
            cam_colored[:, :, 0] = cam  # Red channel
